#!/usr/bin/env python3
"""
Presentación E1 v3 - Dark mode con contenido v2.
Tema: fondo oscuro DATAOILERS con acentos azul/teal.

Uso: python3 presentacion_E1_v3.py
"""
...
# ─── Paleta DARK ───
BG = RGBColor(0x0F, 0x1B, 0x2D)          # Fondo general (navy oscuro)
BG_WHITE = RGBColor(0x1A, 0x2A, 0x3A)    # Tarjetas (card oscura)
BG_SOFT = RGBColor(0x14, 0x20, 0x35)     # Cards secundarias
BORDER = RGBColor(0x2A, 0x3A, 0x4A)      # Bordes suaves
DARK = RGBColor(0xFF, 0xFF, 0xFF)        # Títulos (blanco)
BODY = RGBColor(0x8F, 0xA8, 0xBF)        # Texto cuerpo (gris claro)
SUBTLE = RGBColor(0x5A, 0x7A, 0x94)      # Texto secundario
BLUE = RGBColor(0x34, 0x98, 0xDB)        # Acento principal
TEAL = RGBColor(0x4A, 0xAD, 0xA4)        # Acento secundario
NAVY = RGBColor(0x0F, 0x1B, 0x2D)        # Fondo portada/cierre
...
# Fondos de tarjeta para tema oscuro (tintes sutiles)
CARD_BLUE = RGBColor(0x12, 0x23, 0x3D)
CARD_TEAL = RGBColor(0x11, 0x25, 0x30)
CARD_ORANGE = RGBColor(0x2A, 0x1A, 0x0F)
CARD_GREEN = RGBColor(0x12, 0x25, 0x1A)
